	    ...
	    show: function(options){
	        ...
	        new Model('pos.config').call(
	                ...
	                {timeout:1000}).done(function(){
                            new Model('pos.order').call(
                                    'get_ticket_for_reprint',
                                    [],
                                    undefined,
                                    {timeout:3000}).done(function(tickets){
                                    for(var i in tickets){
                                        $("#tickets").append('<option value="'+tickets[i].id+'">'+'   '
                                        +tickets[i].pos_reference+'  -  '
                                        +self.get_correct_datetime(tickets[i].date_order)+ '</option>');
                                    }
                                }).fail(function(){
            ...
            self.$('#accept').on('click',function(){
                    ...
                    new Model('pos.order').call(
                            ...
                            [$( "#tickets").children("option:selected").val()],
                            undefined,
                            {timeout:5000}).done(function(data){
                                    ...
                                    //head
                                        ...
                                        //date_reprint : date_reprint.getDate()+'/'+(parseInt(date_reprint.getMonth())+1)
                                        //+'/'+date_reprint.getFullYear()+'   '+date_reprint.getHours()
                        ...
                        //console.log('without information reprint')
                    });
