...
odoo.define('pos_speed_up.indexedDB', function (require) {
    ...
    var exports = {
        ...
        update_all: function (_name, objects) {
            var self = this;
            $.when(this.get_object_store(_name))
                .done(function (store) {
                    _.each(objects, function (object) {
                        self.update(store, object);
                    });
                })
                .fail(function (error) {
                    console.log(error);
        ...
        update: function (store, object) {
            var request = store.get(object.id);
            request.onsuccess = function (ev) {
                var item = request.result;
                store.put(_.extend(item, object)).onerror = function (ev) {
                    console.log(ev);
                }
            };
